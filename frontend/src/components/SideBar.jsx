import { BookOpenIcon, HomeIcon, FolderGit2, UsersIcon, BriefcaseBusiness } from "lucide-react";
...
const SidebarItems = [
  ...
  { label: "Jobs", location: "/jobs", icon: <BriefcaseBusiness /> },
];
          ...
          <nav className="flex flex-col gap-4 mt-20">
            {SidebarItems.map((item, index) => (
              <Link key={index} to={item.location}>
                <div
                  className={`flex items-center p-2 rounded-md hover:bg-gray-100 ${activeItem === item.label ? "bg-gray-100" : ""}`}
                >
                  {item.icon}
